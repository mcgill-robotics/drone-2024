...
def read_letter_and_color_on_shape(letter_crop):
    ...
    thresh_im_letter = cv2.adaptiveThreshold(grayImage_letter,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,11,2)
    hsv_img = cv2.cvtColor(letter_crop, cv2.COLOR_BGR2HSV)

    colors = {}
    
    detection = reader.readtext(thresh_im_letter)
